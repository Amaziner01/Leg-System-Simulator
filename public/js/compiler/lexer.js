...
export class Lexer {
    ...
    next() {
        ...
        if (this.currChar == ',') {
            this.advance();
            return new BaseToken(TokType.Comma);
        }
        if (this.currChar == '\n') {
            this.advance();
            this.lineNo++;
            return new BaseToken(TokType.NewLine);
        }
        if (/[0-9]/.test(this.currChar))
            return new Token(TokType.Number, this.number());
        if (this.currChar == '\"') {
            this.advance();
            let str = "";
            while (this.currChar != '\"') {
                if (this.currChar == undefined)
                    throw new Error(`Expected final quotation mark. Line: ${this.lineNo}.`);
                str += this.currChar;
                this.advance();
            }
            this.advance();
            return new Token(TokType.Bytes, str);
        }
        if (this.currChar == undefined) {
        ...
        if (/[A-Za-z_]/.test(this.currChar)) {
            ...
            if (/^r[0-9]+/.test(str)) {
                ...
                if (num >= 16)
                    throw new Error(`Max register is 15. Line: ${this.lineNo}.`);
                return new Token(TokType.Register, num);
            }
            switch (str) {
                ...
                default:
                    if (this.currChar == ':') {
                        this.advance();
                        return new Token(TokType.Label, str);
                    }
                    return new Token(TokType.LabelDeref, str);
            }
        }
